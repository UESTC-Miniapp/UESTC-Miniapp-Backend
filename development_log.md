...
idas更新了，现在登录提交的表单中密码会经过加密，加密方式应该是`AES-128-CBC`，然后用base64编码。

密钥就在获取的HTML上，一个script标签里面，变量名是`pwdDefaultEncryptSalt`，那个标签里面还有个`secure="false"`我不是很清楚什么意思，貌似也没有用到。
...
全部重构完毕，暂时没有接入研究生系统。顺带修复了`grade.php`无法获取成绩总览的bug和`check_token.php`无法检测教务处的bug。
